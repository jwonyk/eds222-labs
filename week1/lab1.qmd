...
# What would I change to get the *column* proportions
penguins %>% 
  count(species, island) %>% 
  group_by(island) %>% 
  mutate(prop = n / sum(n))
...
ggplot(penguins, aes(species, fills = island)) +
  geom_bar()
...
The difference is in the bin use (no bin for bar plot). In bar plot they use catagorical data, histogram is continuous.
