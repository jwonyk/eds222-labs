...
```{r}
#| label: read-data
water <- read_csv("week7/drinking_water.csv",
...
| Column | Meaning |
|----|----|
| `pwsid` | (chr) - Public Water System ID |
| `pwsname` | (chr) - Name of public water system |
| `pctpov` | (dbl) - Percentage of poverty population |
| `pcthisp` | (dbl) - Percentage of Hispanic population |
| `health` | (dbl) - Health risk scores? |
| `medianincomehouse` | (dbl) - Median Household Income |
| `violation` | (lgl) - T / F logical response of water quality violation |
| `pcthisp_bin` | (chr) - Bin grouping of `pcthisp` variable range |
| `pctpov_bin` | (chr) - Bin grouping the \`pctpov\` variable range |
...
# Calculate proportion of violations by Hispanic population bin
figure_1 <- water %>% 
  group_by(pctpov_bin) %>% 
  summarise(prop_violation = mean(violation, na.rm = TRUE))

# Scatter plot
fig1 <- ggplot(figure_1, aes(pctpov_bin, y = prop_violation)) +
  geom_point(size = 3) +
  geom_line(group = 1) +
  labs(title = "Proportion of SDWA Violations by Percent Hispanic Bin",
       x = "Percent Hispanic (binned)",
       y = "Proportion of Systems with Violations") +
  clean_theme

# Calculate proportion of violations by poverty population bin
 figure_2 <- water %>% 
  group_by(pcthisp_bin) %>% 
  summarise(prop_violation = mean(violation, na.rm = TRUE))

# Scatter plot
fig2 <- ggplot(figure_2, aes(pcthisp_bin, y = prop_violation)) +
  geom_point(size = 3) +
  geom_line(group = 1) +
  labs(title = "Proportion of SDWA Violations by Percent Poverty Population Bin",
       x = "Percent Poverty Population (binned)",
       y = "Proportion of Systems with Violations") +
  clean_theme

# Scatter plot
fig3 <- ggplot(water, aes(pcthisp, y = pctpov)) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "yellow") +
  labs(title = "Relationship Between Percent Hispanic and Poverty Population",
       x = "Percent Hispanic",
       y = "Percent Poverty Population") +
  clean_theme

(fig1 / fig2)

ggsave("output/sdwa_vertical.png", width = 8, height = 12, dpi = 300)
```
```{r}
#| label: fig-explore-cont

fig3

ggsave("output/sdwa_relationship.png", width = 12, height = 8, dpi = 300)
