...
beta_0: estimated gage height when no percipitation
...
y_bar <- mean(storm_data$gage_ht) # Fixed number
x <- storm_data$precip_wk_in # Predictor Variable
y_hat <- 3.9 + 2.2 * x #Estimator
...
Having precipitation reduces the error of our prediction by about 55%

Recall that the purpose of a model is to *explain variance*.

**Q6: What's the estimated value of** $\hat \sigma$ **for the model? What's the value of** $\sigma$ **for the response variable itself? Say you needed to predict Los Angeles River gage height following a storm - what does the relationship between these two numbers say about your ability to make that prediction accurately? -\> Try myself!**
